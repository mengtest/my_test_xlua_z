...
static int signkey_len = 64;
static unsigned char signkey[64] = {0x50, 0x51, 0x70, 0x47, 0x69, 0x48, 0x43, 0x43, 0x36, 0x4a, 0x75, 0x74, 0x69, 0x50, 0x70, 0x64, 0x49, 0x62, 0x7a, 0x4d, 0x57, 0x61, 0x50, 0x6c, 0x4d, 0x67, 0x48, 0x70, 0x43, 0x31, 0x6a, 0x50, 0x6e, 0x44, 0x71, 0x6c, 0x65, 0x5a, 0x48, 0x46, 0x62, 0x4b, 0x6a, 0x6d, 0x49, 0x4e, 0x54, 0x62, 0x59, 0x68, 0x50, 0x7a, 0x38, 0x5a, 0x71, 0x79, 0x32, 0x4e, 0x78, 0x57, 0x6c, 0x43, 0x49, 0x36};
static int lua_makesign(lua_State *L)
{
	...
	int i;
	for (i = 0; i < signkey_len; i++) {
		int k = dataLen - signkey_len + i;
		data[k] = signkey[i];
	}
...
static int luakey_len = 32;
static unsigned char luakey[] = {0x55, 0x31, 0x35, 0x61, 0x43, 0x44, 0x56, 0x30, 0x69, 0x6f, 0x6c, 0x54, 0x33, 0x6c, 0x43, 0x36, 0x4d, 0x46, 0x74, 0x49, 0x31, 0x76, 0x71, 0x41, 0x31, 0x48, 0x59, 0x78, 0x7a, 0x52, 0x68, 0x75};
LUA_API void* encrypt_lua(unsigned char* data, int data_len, unsigned char* output, int* outlen) {
	*outlen = rc4_encrypt((const char*)data, data_len, (const char*)luakey, luakey_len, output);
	return (void*)output;
}
LUA_API void* decrypt_lua(unsigned char* data, int data_len, unsigned char* output, int* outlen) {
	*outlen = rc4_decrypt((const char*)data, data_len, (const char*)luakey, luakey_len, output);
	return (void*)output;
}
